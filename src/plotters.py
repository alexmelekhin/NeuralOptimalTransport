...
def plot_latent_Z_images(XZ, Y, T, decoder):
    freeze(T)
    freeze(decoder)
    with torch.no_grad():
        T_XZ = T(
        ...
        XZ_decoded = decoder.decode(XZ[:,0,:Y.shape[1]].squeeze()).sample
        T_XZ_decoded = decoder.decode(T_XZ).sample
        Y_decoded = decoder.decode(Y).sample
        imgs = torch.cat([XZ_decoded, T_XZ_decoded, Y_decoded]).to('cpu').permute(0,2,3,1).mul(0.5).add(0.5).numpy().clip(0,1)
...
def plot_random_latent_Z_images(X_sampler, ZC, Z_STD, Y_sampler, T, decoder):
    ...
    X = X_sampler.sample(10)
    Y = Y_sampler.sample(10)
    return plot_latent_Z_images(XZ, Y, T, decoder)
